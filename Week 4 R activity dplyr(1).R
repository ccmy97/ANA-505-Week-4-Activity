...
haireyecolor1 <- as.data.frame(HairEyeColor)
is.data.frame(haireyecolor1)
select(haireyecolor1,c(Hair,Sex))

#Let's name the dataset with just the two columns, Hair and Sex
#TASK: Write the function to save the two columns as a new dataset
#and give it a name
Hair_Sex <- select(haireyecolor1,Hair,Sex)
dim(Hair_Sex)

#Let's get rid of the Sex column in the new dataset created above
#TASK: Write the function that deselects the sex column
#(hint: use the 'select' function to not select a -column)
select(Hair_Sex,Hair)
...
GenderHair <- rename(Hair_Sex, gender = Sex)

#Let's 'filter' just the males from our dataset
#TASK: Write the function that includes only rows that are 'male'
Male <- filter(GenderHair, gender == "Male")
...
total=mutate(haireyecolor1, total=cumsum(Freq))


#Since we have a males dataset, let's make a females dataset
#TASK: Write the function that includes only rows that are 'female'
Female <- filter(GenderHair, gender == "Female")
